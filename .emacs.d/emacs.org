...
** DONE split buffer and open scratch (e.g. for e-mail or slack message composition)
...
(spaceline-define-segment mute-segment
  ...
  (if (string= system-type "darwin") "-" ; do not include the mute-segment on mac os, because it is very slow
...
(defun ak/org-babel-confirm (lang body)
  (not (or (string= lang "elisp") (string= lang "mermaid"))))
