...
#+end_src
** Load osawm if it exists on disk
#+begin_src elisp :results none
(when (file-exists-p "~/repos/andykuszyk/osawm.el/")
  (load-file "~/repos/andykuszyk/osawm.el/osawm.el")
  (setq osawm-automatically-update-windows nil)
  (global-set-key (kbd "C-x A W s") #'osawm-spotify-toggle-play-pause)
  (global-set-key (kbd "C-x A W o") #'osawm-open-chrome)
  (global-set-key (kbd "C-x A W S") #'osawm-search-chrome))
#+end_src
** Set PKG_CONFIG_PATH
Setting this variable fixes some problems with the compilation of pdf tools on Mac OS:
#+begin_src elisp :results none
(setenv "PKG_CONFIG_PATH"
	(format
	 "/opt/homebrew/lib/pkgconfig:%s"
