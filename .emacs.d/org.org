...
(defun ak/gcal-to-org-format-time (time-string)
  (when (string-match "\\(.*\\) \\– \\(.*\\)" time-string)
    (let* ((from-time (ak/gcal-to-org-clean-from-time (match-string 1 time-string)))
	   (to-time (match-string 2 time-string)))
    ...
    (while (< (line-number-at-pos) (count-lines (point-min) (point-max)))
      (let* ((current-line
	      (buffer-substring-no-properties
	       ...
	       (line-end-position))))
	(if (string-match "^[0-9][0-9]$" current-line)
	    (setq current-day (match-string 0 current-line))
