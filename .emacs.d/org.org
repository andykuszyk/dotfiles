...
(define-key org-mode-map (kbd "M-p") #'org-up-element)
(define-key org-mode-map (kbd "M-n") #'org-down-element)
(require 'org-agenda)
...
#+end_src
* Custom functions and keybindings for navigating description lists
#+begin_src elisp :results none
(defun ak/next-description-list-item ()
  (interactive)
  (re-search-forward "^.* \:\:"))

(defun ak/previous-description-list-item ()
  (interactive)
  (re-search-backward "^.* \:\:"))

(define-key org-mode-map (kbd "C-c M-n") #'ak/next-description-list-item)
(define-key org-mode-map (kbd "C-c M-p") #'ak/previous-description-list-item)
#+end_src
* Log timestamps when closing items, and show them in the agenda
#+begin_src elisp :results none
(setq org-log-done 'time) ; log the timestamp when an item is set to DONE
(setq org-agenda-log-mode-items '(closed clock state)) ; show the timestamp of closed items in the agenda
(setq org-agenda-show-log t) ; show log timestamps in the agenda
(setq org-agenda-skip-scheduled-if-done t) ; hide scheduled items that are closed (they will still show at their closed time)
#+end_src
...
(setq org-agenda-custom-commands
      '(("o" "Agenda and objectives"
         ((tags-todo "objective+radar" ((org-agenda-sorting-strategy '(priority-down))))
	  (tags-todo "objective-radar" ((org-agenda-sorting-strategy '(priority-down))))
	  (agenda "" ((org-agenda-span 1)))
	  (tags-todo "CATEGORY=\"goals\"-future-objective" ((org-agenda-sorting-strategy '(priority-down))))
	  (tags-todo "CATEGORY=\"goals\"+future-objective" ((org-agenda-sorting-strategy '(priority-down))))))))
#+end_src
* Only show deadline reminders for tasks on the day they are due
#+begin_src elisp :results none
...
#+end_src
* Hide line numbers in the org agenda
#+begin_src elisp :results none
(defun ak/org-agenda-mode-hook ()
  (display-line-numbers-mode -1))

(add-hook 'org-agenda-mode-hook #'ak/org-agenda-mode-hook)
#+end_src
* Open org buffers from the agenda in a horizontal split
Commented out for now, because it wasn't quite the behaviour I was looking for.
#+begin_src elisp :results none
; (add-to-list 'display-buffer-alist '(".*org.*+" display-buffer-below-selected))
#+end_src
* Customise "scheduled" text in agenda
#+begin_src elisp :results none
(setq org-agenda-scheduled-leaders '("0d"
				     "%dd"))
#+end_src
* Show recursive cookies for checkboxes
#+begin_src elisp :results none
(setq org-checkbox-hierarchical-statistics nil)
#+end_src
* Custom function to set in-progress status on checkboxes
#+begin_src elisp :results none
(defun ak/org-mode-set-checkbox-in-progress ()
  (interactive)
  (replace-string "- [ ] " "- [-] " nil (pos-bol) (pos-eol)))

(define-key org-mode-map (kbd "C-c C--") #'ak/org-mode-set-checkbox-in-progress)
#+end_src
* Custom org agenda keybinding for <tab> that goes to, and also narrows to heading
#+begin_src elisp :results none
(defun ak/org-agenda-goto ()
  (interactive)
  (org-agenda-goto)
  (org-narrow-to-subtree))

(define-key org-agenda-mode-map (kbd "<tab>") #'ak/org-agenda-goto)
#+end_src
