...
(defun ak/insert-bash-source-block ()
  (interactive)
  (insert "#+begin_src bash :results output verbatim :noweb yes
,#+end_src"))
...
(defun ak/insert-plantuml-source-block (file-name)
  ...
  (insert (format "#+begin_src plantuml :file %s-%s.png :noweb yes

,#+end_src" (buffer-name (current-buffer)) (string-replace " " "-" file-name))))
...
#+begin_src elisp :results none
(defun ak/open-wallpaper ()
  (interactive)
  (find-file (seq-random-elt (seq-filter (lambda (s) (or (s-ends-with-p ".png" s) (s-ends-with-p ".jpg" s))) (directory-files "~/repos/andykuszyk/dotfiles/wallpaper/" t)))))
#+end_src
** Function to format a slack message to request a review on a pr
#+begin_src elisp :results none
(defun ak/format-pr-slack-message (url desc)
  (interactive "MPR URL: \nMwhich... ")
  (kill-new
   (format
    "Hi team 👋

Please could I ask for a review on [this PR](%s) which %s 🙏"
    url
    desc)))
#+end_src
** Function to add a cookie at the end of an org mode heading
#+begin_src elisp :results none
(defun ak/add-org-mode-cookie ()
  (interactive)
  (end-of-line)
  (search-backward " ")
  (insert " [/]")
  (beginning-of-line))
#+end_src
** Function to replace characters in region with bold unicode variants
#+begin_src elisp :results none
(defun ak/replace-region-bold ()
  (interactive)
  (when (use-region-p)
    (let* ((start (region-beginning))
           ...
           (result (mapconcat
                    (lambda (c)
                      (cond
                       ((and (>= c ?a) (<= c ?z)) (string (+ c #x1D3B9)))
                       ((and (>= c ?A) (<= c ?Z)) (string (+ c #x1D3BF)))
                       (t (string c))))
                    text
                    "")))
      ...
      (insert result))))
#+end_src
** Function to replace characters in region with italic unicode variants
#+begin_src elisp :results none
(defun ak/replace-region-italics ()
  (interactive)
  (when (use-region-p)
    (let* ((start (region-beginning))
           (end (region-end))
           ...
           (result (mapconcat
                    (lambda (c)
                      (cond
                       ((= c ?h) (string #x210E))
                       ((and (>= c ?a) (<= c ?z)) (string (+ c #x1D3ED)))
                       ((and (>= c ?A) (<= c ?Z)) (string (+ c #x1D3F3)))
                       (t (string c))))
                    text
                    "")))
      (delete-region start end)
      (insert result))))
#+end_src
** Function to copy a file's absolute file path in a dired buffer
This function allows you to copy a file's absolute file path by pressing =W= in a dired buffer:
#+begin_src elisp :results none
(defun ak/dired-kill-absolute-path ()
  (interactive)
  (let* ((filename (dired-get-filename)))
    (kill-new filename)
    (message filename)))

(define-key dired-mode-map (kbd "W") #'ak/dired-kill-absolute-path)
#+end_src
** Enable frame undelete mode
This will mean that frames can be recovered if they are accidently removed:
#+begin_src elisp :results none
